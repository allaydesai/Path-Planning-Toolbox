...
HBF::maze_path HBF::search(vector< vector<int> > grid, vector<double> start, vector<int> goal) {
	...
	*/
	...
	cout << "vector closed size: " << "(" << closed.size() << ", " << closed[0].size() << ", " << closed[0][0].size() << ")" << endl;
	cout << "vector came_from size: " << "(" << came_from.size() << ", " << came_from[0].size() << ", " << came_from[0][0].size() << ")" << endl;
	...
	// update current state in closed matrix along with the state vals in came_from
	closed[stack][idx(state.x)][idx(state.y)] = 1;
	came_from[stack][idx(state.x)][idx(state.y)] = state;
	int total_closed = 1;

	// create a list of opened states
	vector<maze_s> opened = { state };
	bool finished = false;

	while (!opened.empty())
	{
		// sort opened list based on f value
		...
		for (int i = 0; i < next_state.size(); i++)
		{
			...
			// check if not already closed and verify grid to ensure not an obsticle location
